    ...
    pub fn compute_hash(&mut self) {
        ...
        let result = match self.input_mode {
            InputMode::Text => {
                ...
                self.forge
                    .hash_text(&self.input_text, self.selected_algorithm, salt, iterations)
            }
            InputMode::File => {
                ...
                if self.selected_algorithm.is_password_hash() {
                    ...
                    ui.collapsing("Advanced Options", |ui| {
                        ...
                        ui.horizontal(|ui| {
                            ui.label("Iterations:");
                            let response = ui
                                .add(egui::DragValue::new(&mut self.iterations).range(1..=100000));
                            if response.changed() && self.auto_compute && self.can_compute() {
                                self.clear_results();
                                self.compute_hash();
                            }
...
const AVAILABLE_OUTPUT_FORMATS: [OutputFormat; 2] = [OutputFormat::Hex, OutputFormat::Base64];
